...
Data can only reach its full value when communicated well to the audience. In a crisis situation, people are thirsty for information, and clear data communication becomes even more crucial. Communicating data as tables might be the easiest for data providers, data visualisation helps to convey the trends and situations better for people.
...
- Consider how screen readers interact with visualisations on the web. Is there a text equivalent to images that can be heard by blind and partially-sighted users?
...
- The European Centre for Disease Prevention and Control (ECDC) has written [“Guidelines for presentation of surveillance](https://www.ecdc.europa.eu/en/publications-data/guidelines-presentation-surveillance-data)
...
- Multiple tools can be used to assess accessibility on the web, e.g. [WAVE](WAVE Web Accessibility Evaluation ToolsWAVE Web Accessibility Evaluation Toolshttps://wave.webaim.org), but it is important to check the associated legislation carefully.
...
- On web-based, dynamic data dashboards, the date should be provided within a box/alert above/beside a visualisation. For examples, see; [COVID-19 Risk event tool](https://covid19risk.biosci.gatech.edu/), and [Data dashboards on the Swedish Pathogens Portal](https://www.pathogens.se/dashboards/).
...
- [Decision tree](https://www.data-to-viz.com/) guide toward your graphic possibilities
...
It is recommended that amount/account data is represented in one of three types of visualisation: bar charts (column charts), scatter plots (dot plots), and heatmaps. Here are some considerations when using these plot types with amount/count data:
...
Below is the same clade versus Estonian counties data, visualised as two plot types and colour schemes.

{% include image.html file="data-communication/maakond_yellow.png" caption="" alt="" max-width="10" %}

{% include image.html file="data-communication/maakond_blue.png" caption="" alt="" max-width="10" %}
